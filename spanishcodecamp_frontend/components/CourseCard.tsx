...
const CourseCard: FC<CourseCard> = ({ image, label, description, level }) => {
  return (
    <div className="flex flex-col cursor-pointer text-textColor w-[300px] h-[250px] bg-complement rounded-xl p-4 gap-4">
      <section className="flex items-center justify-between text-center">
        ...
        <span className="text-[1.5rem] text-white">{label}</span>
      </section>
      <section>
        <span>{level}</span>
      </section>
